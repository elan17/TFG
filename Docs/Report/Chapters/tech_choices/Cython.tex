...
implementation has it's flaws, mainly because of it's performance issues. As the
...
be simpler to just use Numpy methods to implement our algorythms (which was done
indeed did in the general autocorrelation function with Wiener–Khinchin's
algorythm). The problem raises when it is needed to access the Numpy array in an
undefined way by the library and to code a loop to compute a function
(the composite autocorrelation for example).\\
